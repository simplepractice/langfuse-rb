...
RSpec.describe Langfuse::Client do
  ...
  describe "#auth_check!" do
    ...
    it "raises ApiError when the credentials have no project" do
      stub_request(:get, projects_url)
        .to_return(
          status: 200,
          body: { "data" => [] }.to_json,
      ...
      expect { client.auth_check! }.to raise_error(
        Langfuse::ApiError,
        "Authentication succeeded but no project is available"
      )
    end

    it "raises UnauthorizedError when Langfuse rejects the credentials" do
      stub_request(:get, projects_url)
        .to_return(status: 401, body: { message: "Invalid credentials" }.to_json)

      expect { client.auth_check! }.to raise_error(Langfuse::UnauthorizedError)
    end
  end
